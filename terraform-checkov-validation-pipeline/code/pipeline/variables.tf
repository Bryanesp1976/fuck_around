...
variable "unique_label" {
  type        = string
  description = "Enter a unique label for your pipeline resources in the format following format: Label-"
}

variable "pipeline_state_bucket_name" {
  type        = string
  description = "Enter bucket name to store terraform state: "
}

variable "pipeline_locks_ddb_table_name" {
  type        = string
