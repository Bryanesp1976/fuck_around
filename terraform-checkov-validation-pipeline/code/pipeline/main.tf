...
resource "aws_s3_bucket" "pipeline_terraform_state" {
  bucket =  "${var.unique_label}-${var.pipeline_state_bucket_name}"
}
resource "aws_s3_bucket_server_side_encryption_configuration" "terraform_state_bucket_encryption" {
  bucket = aws_s3_bucket.pipeline_terraform_state.bucket
  rule {
...
resource "aws_dynamodb_table" "pipeline_terraform_locks" {
  name         = "${var.unique_label}-${var.pipeline_locks_ddb_table_name}"
...
resource "aws_codecommit_repository" "codecommit_repository" {
  repository_name = "${var.unique_label}-${var.codecommit_repository_name}"
  description     = "This respository stores terraform scripts for infrastructure provisioning"

}

resource "aws_iam_role" "codebuild_service_role" {
  name                = "${var.unique_label}-${var.codebuild_service_role_name}"
...
resource "aws_s3_bucket" "terraform_validation_result" {
  bucket = "${var.unique_label}-${var.terraform_validation_result_bucket_name}"
}
...
resource "aws_codebuild_project" "terraform_validation" {
  name         = "${var.unique_label}-${var.terraform_validation_project_name}"
...
resource "aws_codebuild_project" "terraform_deploy" {
  name         = "${var.unique_label}-${var.codebuild_deploy_project_name}"
...
resource "aws_iam_role" "codepipeline_service_role" {
  name                = "${var.unique_label}-${var.codepipeline_service_role_name}"
  assume_role_policy  = data.aws_iam_policy_document.codepipeline_service_role_trust.json
  managed_policy_arns = ["arn:aws:iam::aws:policy/AWSCodePipeline_FullAccess","arn:aws:iam::aws:policy/AmazonEventBridgeFullAccess","arn:aws:iam::aws:policy/AWSCodeCommitFullAccess", "arn:aws:iam::aws:policy/IAMFullAccess", "arn:aws:iam::aws:policy/AWSCodeBuildAdminAccess", "arn:aws:iam::aws:policy/AmazonSNSFullAccess", "arn:aws:iam::aws:policy/AmazonS3FullAccess"]
}
...
resource "aws_s3_bucket" "codepipeline_artifact" {
  bucket = "${var.unique_label}-${var.codepipeline_artifact_bucket}"
}
...
resource "aws_sns_topic" "approval_request" {
  display_name = "${var.unique_label}-${var.sns_topic_name}"
}
...
resource "aws_codepipeline" "codepipeline" {
  name     = "${var.unique_label}-${var.codepipeline_name}"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        RepositoryName       = "${var.unique_label}-${var.codecommit_repository_name}"
        BranchName           = "main" #change from main to master
...
resource "aws_cloudwatch_event_rule" "codecommit_event_rule" {
  name        = "codecommit_trigger_pipeline"
  description = "This event rule will be triggered when changes are detected in the codecommit repo"
  role_arn    = aws_iam_role.codepipeline_service_role.arn
...
resource "aws_iam_role" "lambda_permission" {
  name                = "${var.unique_label}-${var.lambda_role}"
...
resource "aws_lambda_function" "send_notification" {
  ...
  runtime          = "python3.10" #prev version set to 3.7
  environment {
    variables = {
...
resource "aws_iam_role" "event_permission" {
  name = "${var.unique_label}-${var.cw_event_role}"
